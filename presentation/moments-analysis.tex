...
def central_moments(r, data):
    x_mean = mean(data)
    return sum([(xi - x_mean) ** r for xi in data]) / len(data)
        \end{lstlisting}

        \item \textbf{Implementation \& Output (from moments-analysis.ipynb):}
        \begin{lstlisting}
asr_central_moment_3 = central_moments(3, asr_data) # Returns 5830.607
asr_central_moment_4 = central_moments(4, asr_data) # Returns 949064.310
        \end{lstlisting}
        \textbf{Output:}
        \begin{verbatim}
...
\begin{frame}[fragile]
    ...
    \begin{itemize}
        ...
        \item \textbf{Python Function:}
        \begin{lstlisting}[language=Python]
def raw_moments(r, data):
    return sum([xi ** r for xi in data]) / len(data)
        \end{lstlisting}

        \item \textbf{Implementation \& Output (from moments-analysis.ipynb):}
        \begin{lstlisting}
asr_raw_moment_2 = raw_moments(2, asr_data) # Returns 2885.890
        \end{lstlisting}
        \textbf{Output:}
        \begin{verbatim}
