...
def mean(x):
    return sum(x) / len(x)
        ...
        \begin{lstlisting}
asr_mean = mean(asr_data)  # Returns 47.79
        \end{lstlisting}
...
\begin{frame}[fragile]
    \frametitle{Mode Formula and Implementation}
    \begin{itemize}
        \item \textbf{Formula:}
        \[
        ...
        \begin{itemize}
            \item Where:
            \begin{itemize}
                \item $\text{Frequency}(x)$ = Count of occurrences for value $x$
            \end{itemize}
        \end{itemize}
...
def mode(x):
    ...
    return [k for k, v in freq.items() if v == max_count]
        ...
        \item \textbf{Implementation:}
        \begin{lstlisting}
asr_mode = mode(asr_data)  # Returns [0.0]
        \end{lstlisting}
        
        \item \textbf{Interpretation:} The most frequent value (\SI{0.0}{\perthousand}) indicates potential data quality issues - may represent missing entries or countries with exceptionally low rates requiring validation.
    \end{itemize}
\end{frame}
...
def standard_deviation(x):
    m = mean(x)
    return (sum((xi - m)**2 for xi in x) / (len(x)-1)) ** 0.5
        \end{lstlisting}
        
        \item \textbf{Implementation:}
        \begin{lstlisting}
asr_standard_deviation = standard_deviation(asr_data)  # 24.60
asr_variance = asr_standard_deviation**2  # 605.08
        \end{lstlisting}
        
        \item \textbf{Interpretation:} With SD of \SI{24.60}{\perthousand}, countries' rates typically deviate ±24.6 from the mean. High variance (605.08) confirms substantial global disparities in breast cancer incidence.
    \end{itemize}
\end{frame}
...
def iqr(x):
    sorted_x = sorted(x)
    q1 = n_quartile(sorted_x, 0.25)
    q3 = n_quartile(sorted_x, 0.75)
    return q3 - q1
        \end{lstlisting}
...
def largest(x):
    return max(x)
        ...
        \item \textbf{Implementation:}
        \begin{lstlisting}
asr_smallest = smallest(asr_data)  # 0.0
asr_largest = largest(asr_data)    # 105.42
        \end{lstlisting}
...
def coff_range(L, S):
    return (L - S) / (L + S)
        ...
        \item \textbf{Implementation:}
        \begin{lstlisting}
asr_range = 105.42 - 0.0  # 105.42
asr_coff_range = coff_range(105.42, 0.0)  # 1.0
        \end{lstlisting}
...
\begin{frame}[fragile]
    ...
    \begin{itemize}
        ...
        \item \textbf{Python Functions:}
        \begin{lstlisting}[language=Python]
def coefficient_sd(x, x_mean):
    return standard_deviation(x) / x_mean

def coefficient_variation(x, x_mean):
    return (standard_deviation(x) / x_mean) 
    * 100
        \end{lstlisting}
...
\begin{frame}[fragile]
    ...
    \begin{itemize}
        ...
        \item \textbf{Python Functions:}
        \begin{lstlisting}[language=Python]
def n_quartile(sorted_x, q):
    pos = (len(sorted_x) - 1) * q
    floor_pos = int(pos)
    return sorted_x[floor_pos]

def percentile(sorted_x, p):
    return n_quartile(sorted_x, p/100)
        \end{lstlisting}
